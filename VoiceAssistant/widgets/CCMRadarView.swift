...
@objc @IBDesignable public class CCMRadarView: UIView{
    ...
    @IBInspectable public var waveWidth: CGFloat = 4 {
        didSet{
            initialSetup()
        }
    }
    ...
    public func startScale(){
        if ((HomeView != nil) && animating){
            ...
            HomeView?.layer.addAnimation(scaleAnimation, forKey: "scale")
        }
    }
    ...
    public func stopAnimation() {
        animating = false
        HomeView?.layer.removeAnimationForKey("scale")
...
//            let width = waveWidth - CGFloat(Float(index))
            // 边宽递减
            let width = CGFloat(Float(index)) + 1
            if (width < waveWidth){
            ...
            if (index < 2){
                ...
                sublayer.fillColor =  UIColor(red: 0.153, green: 0.784, blue: 0.788, alpha: 1.00).CGColor;//
            }
